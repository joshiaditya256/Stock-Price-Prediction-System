...
# os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3': This line suppresses TensorFlow warnings and logs, only showing errors.
# numpy (np) and pandas (pd): Libraries for numerical computations and data manipulation.



# minmaxscaler: Scales data to a specified range [min, max]. It also returns the original min and max values of the data for inverse scaling purposes.
...
# inverse_scalar: Reverts the scaled data back to its original range using the provided min and max values.
...
# getColumnsData: Retrieves specified columns from a DataFrame.
# getRequiredColumns: Identifies and returns the names of key columns (date, open, low, high, close, volume) in the DataFrame.
...
# LMS: Trains a model using the Least Mean Squares algorithm.
# lmsPred: A nested function that implements the LMS prediction algorithm.
# x_train: Training data excluding the next few days.
# u, l, N: Learning rate, lookahead period, and number of weights, respectively.
# json: Returns a dictionary containing inputs, outputs, actual, and predicted values.
...
# LSTMAlgorithm: Implements an LSTM neural network for time series prediction.
import keras
